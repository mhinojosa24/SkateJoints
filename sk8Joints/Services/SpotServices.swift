...
struct SpotServices {
    ...
    static func fetchAllSpots(completion: @escaping ([SpotModal]?) -> ()) {
        ...
        ref.observeSingleEvent(of: .value) { (snapshot) in
            ...
            snapshot.children.forEach { (snap) in
                ...
                let spot = try! JSONDecoder().decode(SpotModal.self, withJSONObject: value!)
     ...
     */
    static func saveNewSpot(_ spot: SpotModal, completion: @escaping () -> ()) {
        let ref = Constant.databaseRef()
